...
type
  ...
  private
    ...
    cabEnEdic : TCibFac;  //cabina que se está editando
    DesactRefresco: boolean; //desacttiva el refresco de la grilla
    function CabinaSeleccionada: TCibFac;
    function CeldaSeleccionada: string;
    procedure ModificarCeldaActual;
  public
    grpCab: TCibGFac;  //referencia al grupo de cabinas
...
implementation
{$R *.lfm}
uses CibGFacCabinas;  //Se pone aquí para evitar la referencia circular
...
function TfrmAdminCabinas.CabinaSeleccionada: TCibFac;
...
begin
  ...
  Result := TCibGFacCabinas(grpCab).CabPorNombre(nomb);
end;
...
begin
  ...
  TCibFacCabina(cabEnEdic).IP    := StringGrid1.Cells[1, fsel];
  if cabEnEdic.MsjError<>'' then begin
    MsgErr(cabEnEdic.MsjError);
    RefrescarGrilla;   //porque no se refresca cuando hay diálogo modal
    exit;
  end;
  TCibFacCabina(cabEnEdic).Mac   := StringGrid1.Cells[2, fsel];
  ...
  cabEnEdic.y     := n;
  TCibFacCabina(cabEnEdic).ConConexion := UpCase(StringGrid1.Cells[5, fsel]) = 'V';
  if cabEnEdic.MsjError<>'' then begin
    MsgErr(cabEnEdic.MsjError);
    RefrescarGrilla;   //porque no se refresca cuando hay diálogo modal
    exit;
...
begin
  ...
  while grpCab.ItemPorNombre(nomb) <> nil do begin
    Inc(idx);
    nomb := 'Cab' + IntToStr(idx);
  end;
  //agrega
  DesactRefresco := true;  //para evitar muchos refrescos
  cab := TCibGFacCabinas(grpCab).Agregar(nomb,'');
...
begin
  ...
  res := TCibGFacCabinas(grpCab).Eliminar(nom);
  if not res then
...
var
  cab: TCibFacCabina;
begin
  cab := TCibFacCabina(CabinaSeleccionada);
...
begin
  ...
  for c in grpCab.items do begin
    TCibFacCabina(c).ConConexion:=true;
  end;
  DesactRefresco := false;
  RefrescarGrilla;
end;
...
begin
  DesactRefresco := true;  //para evitar muchos refrescos
  for c in grpCab.items do begin
    TCibFacCabina(c).ConConexion:=false;
  end;
  DesactRefresco := false;
  RefrescarGrilla;
end;
