...
type
  ...
  { TCPItemBoleta }
  TCPItemBoleta = class { TODO : Lo ideal serái crearle un índice a los ítems, para evitar errores por accesos concurrentes a las boletas. }
  ...
  public  //campos calculados
    ...
    function Id: string;
  public
    procedure Assign(src: TCPItemBoleta);
  ...
  TCPItemBoletas = specialize TFPGObjectList<TCPItemBoleta>;   //lista de ítems
  ...
  private
    //lista de ítems
    Fitems  : TCPItemBoletas;
    arcProduc: string;     //Archivo de producto.
    procedure AgregarItemR(r: TCPItemBoleta);
    function GetCadEstado: string;
  public
    OnVentaAgregada: procedure of object;  //evento de venta agregada
    property items: TCPItemBoletas read Fitems;
    procedure Recalcula;
    function RegVenta: string;
    property CadEstado: string read GetCadEstado;
  public  //Información y Operaciones con ítems
    function BuscaItem(const id: string): TCPItemBoleta;
    procedure VentaItem(r: TCPItemBoleta; RegisVenta: Boolean);
    procedure ItemClear;
    procedure ItemAdd(it: TCPItemBoleta; recalcular: boolean=true);
  ...
  public  //eventos generales
    OnCambiaEstado: procedure of object;  //cuando cambia alguna variable de estado de la cabina
    OnCambiaPropied: procedure of object; //cuando cambia alguna variable de propiedad de la cabina
  public
    Boleta : TCPBoleta;  //se considera campo de estado, porque cambia frecuentemente
  ...
  TCPFacturable_list = specialize TFPGObjectList<TCPFacturable>;   //lista de ítems
  ...
  public
    ...
    items : TCPFacturable_list; //lista de objetos facturables
    ...
    creen con la conexión configurada desde eñ inicio. No todos los objetos descendientes
    de TCPGrupoFacturable, tienen que usar este campo. Solo les será útil a los que
  ...
  TCPGrupoFacturable_list = specialize TFPGObjectList<TCPGrupoFacturable>;
...
{ TCPItemBoleta }
function TCPItemBoleta.GetEstadoN: integer;
begin
  Result := Ord(estado);
end;
procedure TCPItemBoleta.SetEstadoN(AValue: integer);
begin
  estado := TItemBoletaEstado(AValue);
end;
function TCPItemBoleta.Id: string;
...
procedure TCPItemBoleta.Assign(src: TCPItemBoleta);
begin
  Index  :=src.Index;
  vfec   :=src.vfec;
  vser   :=src.vser;
...
function TCPItemBoleta.regIBol_AReg: string;
...
function TCPItemBoleta.regIBol_ARegVen: string;
{Convierte registro IBol en cadena, para escribir en el archivo de registro,
...
function TCPItemBoleta.GetCadEstado: string;
begin
...
procedure TCPItemBoleta.SetCadEstado(AValue: string);
var
  b: TStringDynArray;
begin
...
{ TCPBoleta }
procedure TCPBoleta.AgregarItemR(r: TCPItemBoleta); { TODO : Pareceira que no es necesario esta función }
//Agrega un ítem a la boleta. Se le debe pasar el registro
begin
  Fitems.Add(r);   //No está creando al objeto
  r.Index := Fitems.Count-1;
end;
procedure TCPBoleta.Recalcula;
...
function TCPBoleta.BuscaItem(const id: string): TCPItemBoleta;
...
procedure TCPBoleta.VentaItem(r: TCPItemBoleta; RegisVenta: Boolean);
...
procedure TCPBoleta.ItemClear;
begin
  Fitems.Clear;
  Recalcula;  //Es rápido este cálculo, así que no hay problema en hacerlo siempre
end;
procedure TCPBoleta.ItemAdd(it: TCPItemBoleta; recalcular: boolean=true);
begin
  Fitems.Add(it);
  if recalcular then Recalcula;
end;
procedure TCPBoleta.ItemDelete(id: string; recalcular: boolean=true);
var
  it: TCPItemBoleta;
begin
...
function TCPBoleta.ItemCount: integer;
begin
  Result := Fitems.Count;
end;
function TCPBoleta.RegVenta: string;
...
function TCPBoleta.GetCadEstado: string;
...
constructor TCPBoleta.Create;
begin
  Fitems := TCPItemBoletas.Create(true);
end;
destructor TCPBoleta.Destroy;
begin
  Fitems.Destroy;
  inherited Destroy;
end;
{ TCPFacturable }
procedure TCPFacturable.SetNombre(AValue: string);
begin
  if FNombre = AValue then exit;
  FNombre := AValue;
  if OnCambiaPropied<>nil then OnCambiaPropied();
end;
procedure TCPFacturable.Setx(AValue: double);
begin
  if Fx=AValue then exit;
  Fx:=AValue;
  if OnCambiaPropied<>nil then OnCambiaPropied();
end;
procedure TCPFacturable.Sety(AValue: double);
begin
  if Fy=AValue then exit;
  Fy:=AValue;
  if OnCambiaPropied<>nil then OnCambiaPropied();
end;
procedure TCPFacturable.LimpiarBol;
begin
  boleta.ItemClear;
...
constructor TCPFacturable.Create;
begin
  Boleta := TCPBoleta.Create;
end;
destructor TCPFacturable.Destroy;
begin
  Boleta.Destroy;
  inherited Destroy;
end;

procedure TCPGrupoFacturable.SetModoCopia(AValue: boolean);
begin
...
{ TCPGrupoFacturable }
constructor TCPGrupoFacturable.Create(nombre0: string; tipo0: TCPTipGrupoFactur
  );
begin
  items  := TCPFacturable_list.Create(true);
  nombre := nombre0;
  tipo   := tipo0;
end;
destructor TCPGrupoFacturable.Destroy;
begin
  items.Destroy;
  inherited Destroy;
end;
