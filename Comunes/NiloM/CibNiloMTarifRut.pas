{Unidad con definiciones básicas para la tarifiación y el enrutamiento}
unit CibNiloMTarifRut;
{$mode objfpc}{$H+}
interface
uses
  Classes, SysUtils, fgl, Types, strutils, MisUtils, CibNiloMPreproc;
type
...
{ TNiloMTarif }
procedure TNiloMTarif.ActualizarMinMax;
...
function TNiloMTarif.BuscaTarifaI(num: String): regTarifa;
...
function TNiloMTarif.regTarif_DeEdi(r: regTarifa; cad: string): string;
...
procedure TNiloMTarif.VerificaTarifa(r: regTarifa; facCmoneda: double);
...
procedure TNiloMTarif.VerificFinal(var numlin: Integer);
...
function TNiloMTarif.PLogErr(mensaje: string): integer;
begin
  Result := 0;
  if OnLogErr<>nil then Result := OnLogErr(mensaje);
end;

function TNiloMTarif.PLogInf(mensaje: String): integer;
begin
  Result := 0;
  if OnLogInf<>nil then Result := OnLogInf(mensaje);
end;

function TNiloMTarif.BuscaTarifa(num: String): regTarifa;
...
function TNiloMTarif.CargarTarifas(archivo: String; facCmoneda: double
  ): Integer;
...
constructor TNiloMTarif.Create;
begin
...
destructor TNiloMTarif.Destroy;
begin
  tarNula.Destroy;
  tarifas.Destroy;
  tarifasTmp.Destroy;
